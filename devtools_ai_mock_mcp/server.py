#!/usr/bin/env python3
"""
DevTools AI Mock MCP Server - Streamable HTTP Transport

This server mocks the functionality of the dev-tools-ai-server as an MCP server
using the official Streamable HTTP transport which is supported by Claude Desktop.
"""
...
def create_app() -> tuple[Server, Starlette]:
    """Create and configure the MCP server and Starlette app."""
    
    # Create the MCP server
    app = Server("devtools-ai-mock-mcp")
    ...
    async def confirm_command(arguments: dict) -> List[types.TextContent]:
        ...
        if any(word in user_response for word in ["yes", "ok", "correct", "good", "approve", "confirm"]):
            ...
            return [
                types.TextContent(
                    type="text",
                    text="I'm not sure about your response. Please clearly indicate:\n"
    ...
    async def lifespan(app: Starlette) -> AsyncIterator[None]:
        ...
        async with session_manager.run():
            logger.info("DevTools AI Mock MCP server started with StreamableHTTP transport!")
            try:
                yield
            finally:
...
def main_cli():
    """CLI entry point for the package (simple version)."""
    # Get host and port from environment variables or use defaults
    host = os.getenv("MCP_HOST", "127.0.0.1")
    port = int(os.getenv("MCP_PORT", "8000"))
    
    # Create the app
    mcp_app, starlette_app = create_app()
    
    logger.info(f"Starting DevTools AI Mock MCP server on http://{host}:{port}")
    logger.info(f"MCP endpoint: http://{host}:{port}/mcp")
    logger.info("Using official Streamable HTTP transport")
